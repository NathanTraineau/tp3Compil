...
class UPPGVar extends UPPExpr {
    ...
    RTLInst toRTL (ArrayList<Pair<String,PRegister>> locals,
                   ArrayList<String> globals, PRegister reg, RTLInst succ) {
        // to do
        return new RTLSetGVar(this.name,reg,succ);
    }//toRTL
...
class UPPNot extends UPPUnOp {
    ...
    RTLInst toRTL (ArrayList<Pair<String,PRegister>> locals,
                   ArrayList<String> globals, PRegister reg, RTLInst succ) {
        // to do
        PRegister nReg = this.e.getPRegister(locals);
        return new RTLXOri(nReg,reg,succ);
    }//toRTL
...
class UPPCond extends UPPInst {
    ...
    RTLInst toRTL (ArrayList<Pair<String,PRegister>> locals,
                   ArrayList<String> globals, RTLInst succ) {
        // to do
        RTLInst ni1 = this.i1.toRTL(locals,globals,succ);
        RTLInst ni2 = this.i2.toRTL(locals,globals,succ);

        PRegister regCond = this.cond.getPRegister(locals);
...
class UPPProcCall extends UPPInst {
    ...
    RTLInst toRTL (ArrayList<Pair<String,PRegister>> locals,
        ...
        // done
        ArrayList<PRegister> nList = new ArrayList<PRegister>(); 
        
        for(UPPExpr val : this.args) {
            PRegister nVal = val.getPRegister(locals);
            nList.add(nVal);
        }

        return new RTLProcCall(this.callee,nList,succ);
    }//toRTL
